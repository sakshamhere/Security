...
The `Diffie-Hellman` Key Exchange is a method for exchanging secret keys over a non-secure medium without exposing the keys on network like RSA. In this both Client and server ends up with a 'master key' which is never sent accross network and is used to encrypt traffic from both sides.

Amazin it is

The Diffie-Hellman (DH) key agreement method to RSA and is very secure as it provides Perfect Forward Secrecy.
