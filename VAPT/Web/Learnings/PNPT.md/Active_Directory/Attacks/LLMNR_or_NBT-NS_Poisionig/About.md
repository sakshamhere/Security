# Link Local Multi Cast Name Resolution (LLMNR) / NBT-NS (NetBios Name Service`) Poisioning

LLMNR was previously known as `NBT-NS` which is `NetBios Name Service`

They are both used to identify host when `DNS` fails to do so.

Now thw way they identify host is by using `NTLM`/`NTLMv2`  hash which can easily be exploited
